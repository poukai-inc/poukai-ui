...
import {
  IconButtonFixture,
  IconButtonSizeFixture,
  IconButtonVariantFixture,
} from "./IconButton.fixtures";

/* ---------- Default render ---------- */

test("renders as a <button> with type=button", async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" />);
...
test("exposes the aria-label on the button host", async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close dialog" />);
...
test("renders a VisuallyHidden span containing the label", async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Dismiss" />);
...
test('size="sm" resolves to a 32×32 square', async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" size="sm" />);
...
test('size="compact" resolves to a 40×40 square', async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" size="compact" />);
...
test('size="lg" resolves to a 52×52 square', async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" size="lg" />);
...
test('size="md" renders a 20px icon', async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" size="md" />);
  const svg = component.locator("svg");
  await expect(svg).toHaveAttribute("width", "20");
  await expect(svg).toHaveAttribute("height", "20");
});
...
test('variant="secondary" applies the variant-secondary class', async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" variant="secondary" />);
  const className = await component.getAttribute("class");
  expect(className).toMatch(/variant-secondary/);
});

test('variant="ghost" applies the variant-ghost class', async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" variant="ghost" />);
  const className = await component.getAttribute("class");
...
test("disabled state is propagated to the button host", async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" disabled />);
  await expect(component).toBeDisabled();
});

test("fires onClick handler", async ({ mount }) => {
  let clicks = 0;
  const component = await mount(<IconButtonFixture aria-label="Close" onClick={() => clicks++} />);
...
test("merges a consumer-provided className with internal classes", async ({ mount }) => {
  const component = await mount(<IconButtonFixture aria-label="Close" className="custom-x" />);
  const className = await component.getAttribute("class");
  expect(className).toMatch(/variant-primary/);
...
test("forwards arbitrary data-* props to the button host", async ({ mount }) => {
  const component = await mount(
    <IconButtonFixture aria-label="Close" data-testid="close-btn" data-category="chrome" />,
  );
...
test("a11y — default IconButton", async ({ mount, page }) => {
  await mount(<IconButtonFixture aria-label="Close dialog" />);
...
test("a11y — all sizes at primary", async ({ mount, page }) => {
  await mount(<IconButtonSizeFixture />);
  const { violations } = await new AxeBuilder({ page })
    .disableRules(["landmark-one-main", "page-has-heading-one", "region"])
    .analyze();
  expect(violations, JSON.stringify(violations, null, 2)).toEqual([]);
