...
| hook     | `useFieldErrors`             | Validation-lib-agnostic error-state hook for Form; returns `{ errors, setErrors, setFieldError, clearAll }` |
...
| organism | `ToastProvider` / `useToast` | Imperative notification portal: auto-dismiss, four tones, action slot, position variants                    |
